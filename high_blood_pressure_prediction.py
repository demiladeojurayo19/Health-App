...
with open('hbp_model.pkl', 'rb') as file:
    loaded_model = pickle.load(file)
def hbp():
    st.title('High Blood Pressure Prediction')
    ...
    with st.container():
        ...
        death_event = st.radio("What is the death event?", ('Yes', 'No'))
        death_event_value = 1 if death_event == 'Yes' else 0
    ...
    data_collected = {'age': [age], 'anaemia': [anemia_value], 'creatinine_phosphokinase': [creatine],
                      'diabetes': [diabetes_value], 'ejection_fraction': [ejection_fraction],
                       'platelets': [platelet_range],
                      'serum_creatinine': [serum_creatinine], 'serum_sodium': [serum_sodium],
                      'gender': [gender_value], 'smoking': [smoking_value], 'time': [time], 'DEATH_EVENT': death_event_value}
    ...
    if prediction == 1:
        output = 'you have high blood pressure'
    else:
        output = 'you do not have high blood pressure'
